...
import sys
sys.path.append("/home/ycao/DEVELOPMENTS/diffusers/examples/dreambooth")
from meddatasets import load_test_data_coco
...
def load_selected_patches():
    patches_folder = 'dataset/27_dreambooth_output/DreamBoothDataset4Med_inpaint2_512_crop1_mask1_bbox1.25_db2_6x25/inference_images'
    patches_num = 13
...
def main():
    ...
    for img,msk,patch in zip(original_images, masks, patches):
        precision_scope = autocast if opt.precision=="autocast" else nullcontext
        with torch.no_grad():
            with precision_scope("cuda"):
                with model.ema_scope():
                    filename=str(index)+'.png'#os.path.basename(opt.image_path)
                    img_p = img.convert("RGB").resize((512,512))
                    image_tensor = get_tensor()(img_p)
                    image_tensor = image_tensor.unsqueeze(0)
                    ref_p = patch.convert("RGB").resize((224,224))
                    ref_tensor=get_tensor_clip()(ref_p)
                    ref_tensor = ref_tensor.unsqueeze(0)
                    mask=msk.resize((512,512))
